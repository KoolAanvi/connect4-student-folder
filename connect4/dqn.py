...
class DQNConfig:
    ...
    batch_size: int = 64
    replay_capacity: int = 50_000  # M for replay buffer capacity
    min_replay_size: int = 2_000  # minimum replay buffer size before starting optimization
    target_sync_interval: int = 1_000  # C for target network sync
    train_interval: int = 4  # N for environment steps between optimization updates (replay ratio)
    hidden_dim: int = 256
